...
class ResultContainerViewController: UIViewController {
  ...
  override func viewWillAppear(_ animated: Bool) {
    ...
    if let response = self.self.documentAuthenticationResponse {
      ...
      if let portraitImageBase64String = response.biometricsAuthenticationResult?.portraitImage {
        self.portraitImageView.image = CommonUtils.convertBase64ToImage(imageString: portraitImageBase64String)
      }
      
      if let signatureImageBase64String = response.documentAuthenticationResult?.signatureImage {
        self.signatureImageView.image = CommonUtils.convertBase64ToImage(imageString: signatureImageBase64String)
      }
    }
  }
  ...
  override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    if segue.identifier == "containerToDetailsSegue" {
      let resultDetailViewController:ResultViewController = segue.destination as! ResultViewController
